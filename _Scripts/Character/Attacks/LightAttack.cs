...
public class LightAttack : MonoBehaviour, IAttack
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.TryGetComponent(out IHittable hittable))
        {
            ...
            {
                Debug.Log(_controller + " LightAtk got Deflected");
            ...
            {
                Debug.Log(_controller + " LightAtk got Blocked");
                _controller.GetBlocked(_force);
            }
            _hits.Add(hittable);
            hittable.GetHit(hitPos, _damage, _force, _myHittable);
            //Debug.Log(_controller + " LightAtk Hit " + hittable.ToString() + " at " + hitPos + " for " + _damage + "dmg with force of " + _force);
        }
    }
