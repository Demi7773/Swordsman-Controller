...
public class HeavyAttack : MonoBehaviour, IAttack
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.TryGetComponent(out IHittable hittable))
        {
            ...
            {
                Debug.Log(_controller + " HeavyAtk got Deflected");
            ...
            {
                Debug.Log(_controller + " HeavyAtk got Blocked, forcing through");
            }
            _hits.Add(hittable);
            hittable.GetHit(hitPos, _damage, _force, _myHittable);
            //Debug.Log(_controller + " HeavyAtk Hit " + hittable.ToString() + " at " + hitPos + " for " + _damage + "dmg with force of " + _force);
        }
    }
