...
public class JumpingAttack : MonoBehaviour, IAttack
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.TryGetComponent(out IHittable hittable))
        {
            ...
            {
                Debug.Log(_controller + "  JumpingAtk got Deflected");
            ...
            {
                Debug.Log(_controller + "  JumpingAtk got Blocked");
                _controller.GetBlocked(_force);
            }
            _hits.Add(hittable);
            hittable.GetHit(hitPos, _damage, _force, _myHittable);
            Debug.Log(_controller + "  JumpingAtk Hit " + hittable.ToString() + " at " + hitPos + " for " + _damage + "dmg with force of " + _force);
        }
    }
}
